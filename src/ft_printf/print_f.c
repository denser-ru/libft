...
/*   Updated: 2021/04/10 17:40:25 by clashund         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	handle_spaces(t_convs *p_convs, int num_len, int sign_arg)
{
	...
	if (num_len >= p_convs->precision)
		num_spaces = num_spaces + p_convs->width - num_len;
	else
		num_spaces = num_spaces + p_convs->width - p_convs->precision;
	if (p_convs->zero == 1)
		num_spaces = 0;
		...
		char *sval, int num_len, int sign_arg)
{
	...
	if (p_convs->zero == 1)
	{
		...
		while (p_convs->width - num_len - index++ > 0 && ++res)
			write(1, "0", 1);
	}
	ft_putstr(sval);
	return (res + ft_strlen(sval) + comp_out(c, 1, 0));
}
...
size_t	letsprint_f(t_convs *p_convs, va_list *ap)
{
	...
	sign_arg = 1;
	p_convs->precision = comp_out(p_convs->precision == -1, 6,
			p_convs->precision);
	if (p_convs->length == 3)
	...
	if (dval <= 0)
	{
		if (((unsigned short)(*((unsigned short *)(&dval) + 4)) >> 15) == 1)
			sign_arg = 0;
		else
			sign_arg = 1;
		dval = -dval;
