...
/*   Updated: 2021/04/10 12:59:16 by clashund         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	max_of_tree(int precision, int width, int len)
{
	...
	if (precision > 0)
	{
		if (width < 0)
			...
			max = (width > len) * width + !(width > len) * len;
	}
	else
	{
...
size_t	print_c(t_convs *p_convs, char cval)
{
	...
	if (p_convs->precision > 0)
	{
		if (p_convs->width < 0)
			...
			max = (p_convs->width > 1) * p_convs->width + !(p_convs->width > 1);
	}
	if (p_convs->width > 0 && p_convs->width > 1)
		max = p_convs->width;
	else
		max = 1;
	str = ft_strjoin("", &cval);
	if (p_convs->zero && p_convs->minus)
		...
		ch = (p_convs->zero > 0) * '0' + !(p_convs->zero > 0) * ' ';
