...
/*   Updated: 2021/04/10 20:53:53 by clashund         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		unsigned long long val, int num_len, int sign_arg)
{
	...
	num_spaces = (num_len >= p_convs->precision) * (num_spaces + p_convs->width
			- num_len) + !(num_len >= p_convs->precision) * (num_spaces
			+ p_convs->width - p_convs->precision);
		...
		unsigned long long val, int num_len, int sign_arg)
{
	...
	if (p_convs->precision != 0 || (p_convs->precision == 0 && val > 0))
		res += compll_out(p_convs->conversion == 'u', val);
	return (res + comp_out(c > 0, 1, 0));
}
...
size_t	print_diu(t_convs *p_convs, va_list *ap)
{
	...
	if (p_convs->conversion == 'u' && sign_arg == 0)
	{
		...
		ullval = (llval < 0) * -llval + !(llval < 0) * llval;
